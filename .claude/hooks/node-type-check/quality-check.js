...
class TypeScriptConfigCache {
   ...
   * @returns {Promise<string[]>} Array of tsconfig file paths
   */
  async findTsConfigFiles() {
    const configs = [];
    try {
      // Try to use glob if available, fallback to manual search
      const glob = await import('glob');
      const globSync = glob.sync || glob.default?.sync;
      if (globSync) {
        return globSync('tsconfig*.json', { cwd: projectRoot }).map((file) =>
          path.join(projectRoot, file),
        );
      }
      throw new Error('glob.sync not available');
    } catch (e) {
   ...
   */
  async isValid() {
    const configFiles = await this.findTsConfigFiles();
   ...
   * @returns {Promise<string>} Path to appropriate tsconfig file
   */
  async getTsConfigForFile(filePath) {
...
/**
 * Initialize optional dependencies (ESLint, Prettier, TypeScript)
 * @returns {Promise<void>}
 */
async function initializeOptionalDependencies() {
  // Create a require function that resolves from the project root
  // This handles pnpm, yarn, and npm node_modules structures correctly
  ...
  try {
    ...
    log.debug(`ESLint not found in project - will skip ESLint checks: ${e.message}`);
  }

  try {
    prettier = projectRequire('prettier');
  } catch (e) {
    log.debug(`Prettier not found in project - will skip Prettier checks: ${e.message}`);
  }

  try {
    ts = projectRequire('typescript');
  } catch (e) {
    log.debug(`TypeScript not found in project - will skip TypeScript checks: ${e.message}`);
  }
}
...
class QualityChecker {
  ...
  async checkTypeScript() {
    ...
    try {
      ...
      const configPath = await tsConfigCache.getTsConfigForFile(this.filePath);
