...
- Use `pnpm exec tsc --noEmit` on specific files to catch type errors
- Use `pnpm exec eslint --fix` on specific files to catch linting issues
...
- Run `pnpm exec tsc --noEmit` on changed files
...
- Remember: use `ctx.runQuery/Mutation/Action` with `api.*.*` references, not
  direct calls
...
- Always run `pnpm exec eslint --fix` and `pnpm exec tsc --noEmit` on SPECIFIC
