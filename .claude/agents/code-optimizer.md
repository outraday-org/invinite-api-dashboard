...
3. **Apply Project-Specific Conventions**: This project has specific patterns
   ...
   - Use snake_case for hooks and utilities (`use-form-state.ts`)
   - Use PascalCase for components (`UserCard.tsx`)
   - Never use `any` - use `unknown` with type guards instead
   - Avoid `as` coercion except for `as const` literals
   - Always include `args` and `returns` validators in Convex functions
...
7. **Convex-Specific Optimizations**:
   ...
   - Use index naming convention: `by_field1_and_field2`
...
4. **Verification Phase**:
   - Run `pnpm exec eslint --fix <changed-files>` to check linting
