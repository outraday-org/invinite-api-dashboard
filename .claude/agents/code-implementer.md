...
- Hooks & utilities: snake_case (e.g., `use-form-state.ts`,
  `format-currency.ts`)
...
- For backend: use `Doc<"tableName">` and `Id<"tableName">` types
- Co-locate types in `types/` folders next to components

### React Patterns

- Use `@fluentui/react-context-selector` for all contexts to minimize re-renders
- Always memoize context provider values with `useMemo`
...
- Index naming must include all fields: `by_userId_and_projectId`
...
7. **Verify Quality**: After implementation, remind the user to run
   `pnpm run check`
...
5. Remind user to verify with `pnpm run check`
