...
function SidebarProvider({
    ...
    React.useEffect(() => {
        // Keep sidebar state in a cookie.
    ...
    return (
        <SidebarContext.Provider value={contextValue}>
            <div
                className={cn(
                    "group/sidebar-wrapper has-data-[variant=inset]:bg-sidebar flex flex-col w-full h-full",
                    className,
                )}
...
function Sidebar({
    ...
    if (isMobile) {
        return (
            <Sheet onOpenChange={setOpenMobile} open={openMobile} {...props}>
                <SheetContent
                    className="bg-sidebar text-sidebar-foreground top-(--app-topbar-height,0px) w-(--sidebar-width) p-0 [&>button]:hidden"
                    data-mobile="true"
    ...
    return (
        ...
        >
            ...
            <div
                className={cn(
                    "fixed bottom-0 top-(--app-topbar-height,0px) z-10 hidden h-svh w-(--sidebar-width) transition-[left,right,width] duration-200 ease-linear md:flex",
                    side === "left"
...
function SidebarMenuButton({
    ...
    & useRender.ComponentProps<"button"> & VariantProps<typeof sidebarMenuButtonVariants> & {
        isActive?: boolean;
